...
\begin{itemize}
  \item \texttt{al2013\_mapaoficial\_retif}: official CNE 2013 results package, converted from legacy \texttt{.xls} to \texttt{.xlsx};
  \item \texttt{al2017\_mapaoficial\_retif02\_01out2018}: official CNE 2017 results package, converted from legacy \texttt{.xls} to \texttt{.xlsx};
  \item \texttt{2021al\_mapa\_oficial}: official CNE 2021 results package;
  \item \texttt{2025al-mapa-oficial\_retificado}: official CNE 2025 results package including the published correction.
\end{itemize}
...
%% ── Top row ──────────────────────────────────────────────────────────
\node[entity] (elections) {%
  \textbf{elections}\\election\_id PK\\election\_code UQ\\date, source};

\node[entity, right=of elections] (organs) {%
  \textbf{organs}\\organ\_code PK\\CM, AM, AF\\territory level};

\node[entity, right=of organs] (territories) {%
  \textbf{territories}\\territory\_code PK\\level, parent\\geom};

%% ── Middle ───────────────────────────────────────────────────────────
\node[entity, below=1.6cm of organs] (candidacies) {%
  \textbf{candidacies}\\candidacy\_id PK\\election/organ/territory FK\\sigla, type};

%% ── Bottom row ───────────────────────────────────────────────────────
\node[entity, below=1.6cm of candidacies, xshift=-4.2cm] (turnout) {%
  \textbf{turnout\_results}\\election FK\\territory FK\\organ FK\\registered, voters\\blank, null};

\node[entity, below=1.6cm of candidacies] (votes) {%
  \textbf{vote\_results}\\election FK\\territory FK\\organ FK\\candidacy FK\\votes};

\node[entity, below=1.6cm of candidacies, xshift=3.8cm] (seats) {%
  \textbf{seat\_results}\\election FK\\territory FK\\organ FK\\candidacy FK\\percentage, mandates};

\node[entity, right=1.6cm of seats] (elected) {%
  \textbf{elected\_members}\\election FK\\territory FK\\organ FK\\candidacy FK\\position, name};

%% ── Direct edges: top row → candidacies ─────────────────────────────
\draw[rel] (elections.south)  -- (candidacies.west);
\draw[rel] (organs.south)     -- (candidacies.north);
\draw[rel] (territories.south)-- (candidacies.east);

%% ── candidacies → result nodes ───────────────────────────────────────
\draw[rel] (candidacies.south west) -- (turnout.north east);
\draw[rel] (candidacies.south)      -- (votes.north);
\draw[rel] (candidacies.south east) -- (seats.north west);
\draw[rel] (candidacies.east)       -| (elected.north);

%% ── elections / organs / territories → turnout ───────────────────────
%% Route left of the elections box to avoid crossing entities
\draw[rel] (elections.west)
    -- ++(-0.6,0)
    |- (turnout.west);

\draw[rel] (organs.west)
    -- ++(-0.3,0)
    |- ([yshift=0.15cm]turnout.north);

%% Route right of territories box then down
\draw[rel] (territories.east)
    -- ++(0.6,0)
    |- ([yshift=-0.15cm]turnout.north);

\end{tikzpicture}
\caption{Simplified operational ER diagram. Primary entities are elections, organs,
territories, and candidacies; result tables reference these dimensions.}
...
\texttt{AL2021 vote\_results} & 12277\\
\texttt{AL2025 vote\_results} & 12839\\
\texttt{dw.fact\_election\_results} & 49337\\
\bottomrule
\end{tabular}
\end{table}
